...
class MovingAvgStrategy:
    ...
    def __init__(self, tickers:List[str], init_account_size:int, atr_multiplier:int, r_max:float, 
                 max_position_size:float, fast_ma:int, slow_ma:int, atr_periods:int, start:str, 
                 end:str, shorts:bool, leverage:bool) -> None:
        ...
        Parameters:
            ...
            fast_ma (int): Number of lookback periods to generate a moving average.
            slow_ma (int): Number of lookback periods to generate a moving average.
    ...
    def _get_data(self) -> pd.DataFrame:
        ...
        df.drop(['Adj Close', 'Volume'], 
                inplace=True, axis=1)
        df.ffill(inplace=True)
        return df.swaplevel(axis=1)
    
    def _calc_movavgs(self) -> pd.DataFrame:
        """Creates Simple Moving Average indicators for buy/sell signals.
        ...
        for t in self.tickers:
            ...
            self.data[t, 'fast_ma'] = self.data[t]['Close'].rolling(
                self.fast_ma).mean()
            self.data[t, 'slow_ma'] = self.data[t]['Close'].rolling(
                self.slow_ma).mean()
    ...
    def _run_system(self, ticker:str, data:pd.DataFrame, position:dict) -> dict:
        ...
        N = data['N']
        if np.isnan(price) or np.isnan(data['slow_ma']):
            return position
        if np.isnan(N):
            return position      
        ...
        if position is None:
            ...
            if position['long']:
                if data['fast_ma'] < data['slow_ma'] or data['Low'] <= position['stop_price']:
                    self.cash += position['shares'] * price * position['multiplier']
                    position = None
            else:
                if data['fast_ma'] > data['slow_ma'] or data['High'] >= position['stop_price']:
                    self.cash += position['shares'] * price * position['multiplier']
                    position = None
